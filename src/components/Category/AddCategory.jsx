            ...
            <div className="w-full  p-4">
                <div className="flex flex-col justify-center max-w-md mx-auto h-full py-12">
                    <form onSubmit={formik.handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                        ...
                        {/* show loading */}
                        {categoryMutation.isPending && (<AlertMessage type='loading' message='Loading Please Wait' />)}
                        {categoryMutation.isSuccess && (<AlertMessage type='success' message='Category Created Successfully' />)}
                        {categoryMutation.isError && (<AlertMessage type='error' message={categoryMutation?.error?.response?.data?.message} />)}
